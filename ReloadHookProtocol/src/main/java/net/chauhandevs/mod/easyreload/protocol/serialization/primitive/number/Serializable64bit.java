...
public abstract class Serializable64bit<T extends Number> extends PrimitiveSerializableBase<T, Number> {

    @Override
    public byte[] serialize() {
        long val = internalGet().longValue();
    ...
    public void deserialize(byte[] data) {
        ...
        for (int i = 0; i < sl; i++){
            long byteValShifted = ((((long) data[i]) << bss) >>> bss) << (i*8);
